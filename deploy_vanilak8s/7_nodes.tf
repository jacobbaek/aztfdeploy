...
resource "azurerm_linux_virtual_machine" "nodes" {
  ...
  name                  = format("%s-node%s", var.res_prefix, count.index)
  ...
  os_disk {
    name                 = format("%s-nodedisk%s", var.res_prefix, count.index)
    caching              = "ReadWrite"
  ...
  computer_name                   = format("%s-node%s", var.res_prefix, count.index)
